    ...
    public String map(String line) throws Exception {
        ...
        for (String entityId : assets.keySet()) {
            if (StringUtil.equalsIgnoreCase(entityId, srcId)) {
                // 表示是否为内网ip
                json.put("isInternalIp", SystemUtil.isInternalIp(dstIp));
                // 表示与资产匹配的是srcId还是dstId
                json.put("isMatchSrc", true);
                match = true;
                break;
            }
            if (StringUtil.equalsIgnoreCase(entityId, dstId)) {
                json.put("isInternalIp", SystemUtil.isInternalIp(srcIp));
                json.put("isMatchSrc", false);
                match = true;
                break;
            }
        }
        return match ? json.toJSONString() : "";
    }
}
